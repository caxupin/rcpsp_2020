	...
	int numWorkers)
{
    ...
    try {
		...
		*/
		...
		// COMPUTE MINIMUM RESOURCE CONSUMPTION

		min_rhs = new int[nbResources];
		cnt_aux = new int[nbResources];
		aux_coeff = new double[nbResources];
		...
		for(r=0; r < nbResources; r++)
		{
			min_rhs[r] = DBL_MAX;
			for(t=0; t < tmax; t++)
				min_rhs[r] = fmin(min_rhs[r], rhs[r][t];
			cnt_aux[r] = 0;
			for(t=0; t < tmax; t++)
			{
				cnt_aux[r] += ( (min_rhs[r] == rhs[r][t]) ? 1 : 0)
				cnt_tot += 1;
			}
		}
        ...
        IloIntervalVarArray dummy_variables(env, nbTasks);

        for (j=0; j< cnt_tot; j++) 
        {
            dummy_variables[j] = IloIntervalVar(env);
            task_variables[j].setSizeMin(1);
            task_variables[j].setSizeMax(1);
            task_variables[j].setPresent();
        }

		// ADD AUXILIAR VARIABLES
