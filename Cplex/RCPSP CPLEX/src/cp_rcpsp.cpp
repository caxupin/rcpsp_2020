    ...
    int **prhs)
{
    ...
    try 
    {
        ...
        succ = new int*[njobs];
        lag = new int*[njobs];
        lhs = new int*[njobs]; //Trabajos
        rhs = new int[nresources]; //Recursos
...
//Escribe en pantalla la optimizacion
int rcpsp_write(
    int njobs,
    int *duration,        
    int *release_date,    
...
int rcpsp_cp(
	...
	double GapLimit,
	int numWorkers,
    std::vector<std::vector<int>> segper)          // SUBCJTOS DE PERIODOS)
{
    IloEnv env;
...
//-------------------------------------
	//Verifica que solucion inicial sea valida
	...
	fprintf(stderr, "TESTED INPUT SOLUTION AND FOUND %d ERRORS.\n", rval);
	
	//Si solucion inicial no es valida (rval = 0) entonces se sale del problema	
	...
	// Variables de intervalo
        ...
        for (j=0; j<nbTasks; j++)
        {
            IloInt nbSucc = nsucc[j]; //Cantidad de sucesores del trabajo j
            for (IloInt s=0; s<nbSucc; ++s) 
			{
				model.add(IloPresenceOf(env,task_variables[succ[j][s]]) <= IloPresenceOf(env,task_variables[j])); //j debe empezar antes que sus sucesores, IloPresenceOf tiempo de aparicion
                model.add(IloStartBeforeStart(env, task_variables[j], task_variables[succ[j][s]], lag[j][s])); // StartBeforeStart, establece que s(j) + lag(j,s) <= s(succ(j,s))
            }
        }
		...
		*/
		...
		if(latest_start_date)
		{
            for (j=0; j<nbTasks; j++)
				if(latest_start_date[j] > (-1))
					model.add(IloStartOf(task_variables[j]) <= latest_start_date[j]); //Trabajos deben comenzar antes de su due_release
		}
        ...
        for (r=0; r<nbResources; r++)
        {
            ...
            for (j=0; j<nbTasks; j++)
                if (lhs[j][r] > 0) 
                    resource_consumptions[r] += IloPulse(task_variables[j], lhs[j][r]); //Se añade la cantidad de recursos que usa j de r

            model.add(resource_consumptions[r] <= rhs[r]); // no se puede superar la cantidad fija de recursos disponibles.
        }

        // DEFINE OBJECTIVE FUNCTION

		if(obj_type == 1) //1 == makespan
		{
			...
			IloObjective objective = IloMinimize(env,IloMax(ends)); //maximiza los tiempos
			model.add(objective);
		}
		else if(obj_type == 2 || obj_type == 3) // 2 == npv-end || 3 == npv-start
		{
			IloNumExprArray profitsExpr(env, nbTasks); //Constantes. El profit de cada trabajo
			for (j=0; j<nbTasks; j++)
			{
				IloExpr pexp;
				if(obj_type == 2)
					pexp = profit[j] / IloPower(1+discount_rate,IloEndOf(task_variables[j]));
				else
					pexp = profit[j]*IloPower(1+discount_rate,IloStartOf(task_variables[j]));
		...
		else if(obj_type == 4) //No hay tasa de perdida, se maximiza x_jt*p_j
		{
		...
		*out_gap = 1e10;
		...
		if( cp.getInfo(IloCP::NumberOfSolutions) )
		{
		    *out_ubound = cp.getObjBound();
		    *out_solval = cp.getObjValue();
		    *out_gap = cp.getObjGap();
